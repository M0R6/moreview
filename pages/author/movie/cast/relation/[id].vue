...
const checkFilmOwnership = async () => {
  try {
    const response = await fetch(`/api/movie/check/${route.params.id}`);
    if (!response.ok) {
      throw new Error("Failed to fetch film data");
    }
    const filmData = await response.json();
    console.log("Film Data:", filmData);
    if (filmData.createdBy.id !== data.value.user.id) {
      showToast("Access denied", "error");
      setTimeout(() => {
        navigateTo("/");
      }, 1000);
    ...
    console.error("Error checking film ownership:", error);
  }
};

onMounted(async () => {
  await getAuthUser();
  await checkFilmOwnership();
  await fetchCasts();
