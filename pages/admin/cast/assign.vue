<template>
  <v-form @submit.prevent="addCastRelation">
    <v-select v-model="selectedFilm" :items="movies" item-title="title" item-value="id" label="Select Film"></v-select>
    <v-select v-model="selectedCast" :items="castList" item-title="name" item-value="id" label="Select Cast"></v-select>
...
const addCastRelation = async () => {
  try {
    const response = await useFetch(`/api/cast/addRelation`, {
      method: "POST",
      body: {
    ...
    if (!response.ok) {
      throw new Error("Failed to add cast relation");
    }
    const data = await response.json();
    console.log("Added Cast Relation:", data);
    showToast("Cast relation added successfully", "success");
  } catch (error) {
    console.error("Error adding cast relation:", error);
    showToast("Failed to add cast relation", "error");
  }
};
